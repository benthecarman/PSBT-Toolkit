...
import org.bitcoins.crypto.{ECPrivateKey, SchnorrDigitalSignature}
...
object SchnorrSignDataDialog {

  def showAndWait(parentWindow: Window): Option[SchnorrDigitalSignature] = {
    val dialog = new Dialog[Option[SchnorrDigitalSignature]]() {
      initOwner(parentWindow)
      title = "Schnorr Sign Data"
    }
    ...
    dialog.showAndWait() match {
      case Some(Some(sig: SchnorrDigitalSignature)) =>
        Some(sig)
      case Some(_) | None =>
        None
    }
