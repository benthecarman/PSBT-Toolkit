...
object AddSignatureDialog {
  ...
  def showAndWait(
    ...
    okButton.disable <== {
      val indexOpt = indexTF.text.value.toIntOption
      val pubkeyTx = pubKeyTF.text.value
      val pubkeyOpt = Try(ECPublicKey(pubkeyTx)).toOption
      val sigTxt = signatureTF.text.value
      val sigOpt = Try(ECDigitalSignature(sigTxt)).toOption
      BooleanProperty(
        (indexOpt.isEmpty || indexOpt.get > maxIndex) || pubkeyOpt.isEmpty || sigOpt.isEmpty)
    }
